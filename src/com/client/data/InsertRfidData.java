...
public class InsertRfidData {
									...
									if(current_student_rfid!=null){
										...
										if(!current_student_rfid.getEvening_in_bound().equals("no")){
											...
											if(!eve.getIn_bound().equals("no")){
												...
												logger.info("Evening session error [ Evening session is not started yet ] ");
											}
											...
											if(!eve.getIn_bound().equals("no")){
												...
												if(!current_student_rfid.getEvening_out_bound().equals("no")){
													...
													// morningSession=context.getBean("morningSession",MorningSession.class);
													rfid_info rf=context.getBean("rfid_info",rfid_info.class);
													rf.setEvening_out_bound(sdf.format(cal.getTime()));
													Query query = new Query(c1.andOperator(
															 
															  Criteria.where("rfid_info.rfid").is(rfid_num))
															);
													//logger.info("Query --> "+query);
													Update update = new Update().set("rfid_info.$.evening_in_bound", rf.getEvening_out_bound());
										...
										if(!eve.getIn_bound().equals("no")){
											...
											// morningSession=context.getBean("morningSession",MorningSession.class);
											rfid_info rf=context.getBean("rfid_info",rfid_info.class);
											rf.setEvening_in_bound(sdf.format(cal.getTime()));
											Query query = new Query(c1.andOperator(
													 
													  Criteria.where("rfid_info.rfid").is(rfid_num))
													);
											//logger.info("Query --> "+query);
											Update update = new Update().set("rfid_info.$.evening_in_bound", rf.getEvening_in_bound());
						...
						if(student_rfid_info!=null){
							...
							if(current_student_rfid!=null){
								...
								if(!current_student_rfid.getMorning_out_bound().equals("no")){
									...
									// morningSession=context.getBean("morningSession",MorningSession.class);
									rfid_info rf=context.getBean("rfid_info",rfid_info.class);
									rf.setMorning_out_bound(sdf.format(cal.getTime()));
									Query query = new Query(c1.andOperator(
											 
											  Criteria.where("rfid_info.rfid").is(rfid_num))
											);
									//logger.info("Query --> "+query);
									Update update = new Update().set("rfid_info.$.morning_out_bound", rf.getMorning_out_bound());
	...
	public void insertTimeIntoDriver(String rfid_num,ApplicationContext context) {
		...
		try{
			...
			driver_info driverInfo=(driver_info) oneDayBusInfo.getDriver_info();
