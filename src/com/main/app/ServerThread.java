	...
	public void run() {
		...
		try {
			...
			while (line.compareTo("QUIT") != 0) {
				...
				try{
					String arr[]=data.getDataLine().trim().split(",");
					
					if(arr[0].equals("R")){
						
						//String data=arr[1];
						if(arr.length<2 || arr[1].equals("") || arr[1]==null || arr[1].length()<=5){
							logger.info("RFID is empty...");
						}
						else{
						...
						logger.info("Invalid Data [ " + data.getDataLine() + " ]");
					}
				}
				catch(Exception e){
